...
day_summary <- df %>%
  ...
  group_by(SERVICE_ID) %>%
  summarise(
    total_trips = sum(TOTAL_COUN, na.rm = TRUE),
    n_segments = n(),
    avg_trips_per_segment = mean(TOTAL_COUN, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
wk_spatial %>%
  ...
  summarise(
    total_segments = n(),
    pct_night = round(100 * mean(NIGHT_COU > 0, na.rm = TRUE), 1),
